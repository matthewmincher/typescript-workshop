...
export function calculateResistorValue(bands: ResistorInput): number {
  const digitColours = bands.slice(0, 2);
  const multiplier = bands[bands.length - 1];

  const digits = digitColours
    .map((colour) => digitMap[colour])
    .filter((digit) => digit !== undefined)
    .reduce((accumulator, value) => accumulator + value, "");
  const multiplierValue = multiplierMap[multiplier] ?? 1;

  return parseInt(digits) * multiplierValue;
}
